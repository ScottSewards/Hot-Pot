...
if(isset($_POST["submit-change-password"])) {
  ...
  if(password_verify($old_password, $fetch_password['password'])) {
    ...
    mysqli_query($connection, "INSERT INTO user_password_changes (user_id, change_from, change_to, change_date) VALUES ('{$my_id}', '{$old_password}', '{$new_password_hash}', '{$datetime}')");
    head_to_self();
